...
function generateIcons (loader, imageFileStream, pathPrefix, query, callback) {
  ...
  favicons(imageFileStream, {
    ...
    result.images.forEach(function (image) {
      ...
      if (image.name === 'favicon.ico' && query.generateRoot !== undefined) {
        loaderResult.files.push(query.generateRoot + image.name);
        loader.emitFile(query.generateRoot + image.name, image.contents);
      }
    });
